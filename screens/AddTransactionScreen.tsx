    ...
    {options.map((cat) => {
      ...
      return (
        <TouchableOpacity
          ...
          activeOpacity={0.7}
        >
          <Ionicons name={cat.icon as any} size={16} color={isActive ? '#fff' : cat.color} />
          <Text style={[styles.categoryChipText, isActive && { color: '#fff' }]}>{cat.label}</Text>
        </TouchableOpacity>
      );
    })}
  </View>
...
export default function AddTransactionScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        'Sucesso',
        recurring
          ? `Transação recorrente criada! Será repetida todo dia ${recurringDay} de cada mês.`
  ...
  }, [rawAmount, category, description, type, selectedCard, recurring, recurringDay, user, refreshData, navigation]);
      ...
      >
        ...
        <View style={styles.typeTabsContainer}>
          {TYPE_TABS.map((tab) => (
            <TouchableOpacity
              ...
              activeOpacity={0.8}
            >
              <Text style={[styles.typeTabText, type === tab.key && { color: '#fff' }]}>{tab.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        {type !== 'card' && (
          <View style={styles.section}>
            ...
            <CategoryGrid options={categories} selected={category} onSelect={setCategory} />
          </View>
        )}
              ...
              <View style={styles.categoryGrid}>
                {data.cards.map((card) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={card.id}
                      style={[styles.categoryChip, isActive && { backgroundColor: '#f59e0b', borderColor: '#f59e0b' }]}
                      onPress={() => setSelectedCard(card.id)}
                      activeOpacity={0.7}
                    >
                      <Ionicons name="card-outline" size={16} color={isActive ? '#fff' : '#f59e0b'} />
                      <Text style={[styles.categoryChipText, isActive && { color: '#fff' }]}>{card.name}</Text>
                    </TouchableOpacity>
                  );
                })}
              </View>
          ...
          <View style={styles.section}>
            <View style={styles.recurringRow}>
              <View style={styles.recurringLeft}>
                <View style={[styles.recurringIconWrap, { backgroundColor: recurring ? `${activeColor}18` : '#f3f4f6' }]}>
                  <Ionicons name="repeat-outline" size={20} color={recurring ? activeColor : '#9ca3af'} />
                </View>
                <View>
                  <Text style={styles.recurringTitle}>Transação recorrente</Text>
                  <Text style={styles.recurringSubtitle}>Repete automaticamente todo mês</Text>
                </View>
              </View>
              <Switch
                value={recurring}
                onValueChange={setRecurring}
                trackColor={{ false: '#e5e7eb', true: `${activeColor}60` }}
                thumbColor={recurring ? activeColor : '#fff'}
              />
            </View>
            ...
            {recurring && (
              <View style={styles.dayPickerWrap}>
                ...
                >
                  {DAYS.map((d) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.dayChip,
                        recurringDay === d && { backgroundColor: activeColor, borderColor: activeColor },
                      ]}
                      onPress={() => setRecurringDay(d)}
                    >
                      <Text style={[styles.dayChipText, recurringDay === d && { color: '#fff' }]}>
                        {d}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <Text style={styles.dayPickerHint}>
                  Esta transação será gerada automaticamente todo dia {recurringDay} de cada mês.
                </Text>
              </View>
            )}
          </View>
        ...
        >
          {loading ? (
            ...
            <>
              <Ionicons name={recurring ? 'repeat' : 'checkmark-circle-outline'} size={20} color="#fff" />
              <Text style={styles.submitText}>
                {recurring ? 'Salvar Recorrente' : 'Salvar Transação'}
              </Text>
            </>
          )}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  typeTabsContainer: { flexDirection: 'row', gap: 10, marginBottom: 20 },
  typeTab: {
  ...
  amountLabel: { fontSize: 12, color: '#9ca3af', fontWeight: '600', letterSpacing: 1, marginBottom: 8 },
  amountRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  currencySymbol: { fontSize: 24, fontWeight: '700' },
  amountInput: { fontSize: 40, fontWeight: '700', minWidth: 120, textAlign: 'center' },
  ...
  textInput: {
    ...
    paddingVertical: 14, fontSize: 15, color: '#1f2937',
  ...
  recurringTitle: { fontSize: 14, color: '#1f2937', fontWeight: '600' },
  recurringSubtitle: { fontSize: 11, color: '#9ca3af', marginTop: 2 },

  dayPickerWrap: {
    marginTop: 12, backgroundColor: '#fff', borderRadius: 14,
    padding: 14, borderWidth: 1, borderColor: '#e5e7eb',
  },
  dayPickerLabel: { fontSize: 12, color: '#6b7280', fontWeight: '600', marginBottom: 10 },
  dayPickerScroll: { gap: 8, paddingBottom: 4 },
  dayChip: {
